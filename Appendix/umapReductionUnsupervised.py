...
def apply_umap(embeddings, n_neighbors, min_dist, metric, n_components_2d=2, n_components_3d=3, use_scaler=True):
    # Check if embeddings are 3D (e.g., (batch_size, num_chunks, embedding_dim))
    if len(embeddings.shape) == 3:
        embeddings = embeddings.reshape(embeddings.shape[0], -1)  # Flatten to 2D

    # Standardize embeddings (optional, based on use_scaler flag)
    if use_scaler:
        scaler = StandardScaler()
        embeddings = scaler.fit_transform(embeddings)

    # Apply UMAP for 2D and 3D reductions
...
input_file_path = r'C:\Users\kstat\Documents\Dissertation\Data\Methods\Method_2\Embeddings\normalized_embeddings.json'
output_dir = r'C:\Users\kstat\Documents\Dissertation\Data\Methods\Method_2\UMAP'
